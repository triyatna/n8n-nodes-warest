...
export class WARESTTrigger implements INodeType {
  description: INodeTypeDescription = {
    displayName: "WAREST Webhook",
    name: "warestWebhookTrigger",
    group: ["trigger"],
    version: 1,
    description:
      "Receive webhook events from WARest with signature verification",
    defaults: {
      name: "WAREST Webhook",
    },
    codex: {
    ...
    properties: [
      ...
      },
      {
        ...
        name: "allowedEvents",
        type: "multiOptions",
        options: eventOptions.map((value) => ({ name: value, value })),
        default: [],
        description:
          "Leave empty to accept all events. When populated, only matching event tags trigger the workflow.",
      },
      {
  ...
  async webhook(this: IWebhookFunctions): Promise<IWebhookResponseData> {
    ...
    const allowedEventsParam = this.getNodeParameter("allowedEvents", 0) as
      | string[]
      | string
      | undefined;
    ...
    if (
      ...
      (!eventName || !allowedEvents.includes(eventName))
    ) {
      sendResponse(res, {
        mode: responseMode,
        staticPayload,
